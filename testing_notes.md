...
- A matched-lane CTM execute that fails dispatch clears the item, but if it was the last CTM item the queue currently leaves `next_sequence_to_execute` unchanged. Future tests should not assume head advancement on the terminal clear-to-empty transition unless the onchain behavior is changed deliberately.
