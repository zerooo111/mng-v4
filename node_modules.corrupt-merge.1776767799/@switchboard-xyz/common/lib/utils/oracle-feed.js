 ...
 */
const oracleFeedSchema = z.looseObject({
    jobs: z.array(z.any()).refine(jobs => {
        try {
            if (!NonEmptyArrayUtils.safeValidate(jobs)) {
                throw new Error('"jobs" property is expected to be a non-empty array');
            }
            jobs.forEach((job, idx) => {
                try {
                    normalizeOracleJob(job);
                }
                catch (error) {
 ...
 */
export function normalizeOracleFeed(data) {
    const parseFeedObject = (data) => {
        ...
        return OracleFeed.fromObject(validation.data);
    };
 ...
 */
export function serializeOracleFeed(data) {
 ...
 */
export function deserializeOracleFeed(data) {
