...
import { Base, fetchMultipleMintInfos, generatePubKey, getTransferAmountFee, InstructionType, minExpirationTime, } from '../base';
import { getATAAddress } from '../base/pda';
import { getMultipleAccountsInfo, getMultipleAccountsInfoWithCustomFlags, Logger, splitTxAndSigners, TOKEN_PROGRAM_ID, } from '../common';
import { Currency, CurrencyAmount, ONE, Percent, Price, Token, TokenAmount, ZERO } from '../entity';
import { SPL_ACCOUNT_LAYOUT } from '../spl';
import { closePositionInstruction, collectRewardInstruction, createPoolInstruction, decreaseLiquidityInstruction, increasePositionFromBaseInstruction, increasePositionFromLiquidityInstruction, initRewardInstruction, openPositionFromBaseInstruction, openPositionFromLiquidityInstruction, setRewardInstruction, swapInstruction, } from './instrument';
import { ObservationInfoLayout, OperationLayout, PoolInfoLayout, PositionInfoLayout, TickArrayBitmapExtensionLayout, TickArrayLayout, } from './layout';
...
import { getPdaExBitmapAccount, getPdaMetadataKey, getPdaOperationAccount, getPdaPersonalPositionAddress, getPdaPoolId, getPdaPoolRewardVaulId, getPdaPoolVaultId, getPdaProtocolPositionAddress, getPdaTickArrayAddress, } from './utils/pda';
...
export class Clmm extends Base {
    static makeMockPoolInfo({ programId, mint1, mint2, ammConfig, createPoolInstructionSimpleAddress, initialPrice, startTime, owner, }) {
        ...
        return {
            ...
            exBitmapInfo: {
                ...
                positiveTickArrayBitmap: Array.from({ length: EXTENSION_TICKARRAY_BITMAP_SIZE }, () => Array.from({ length: 8 }, () => new BN(0))),
                negativeTickArrayBitmap: Array.from({ length: EXTENSION_TICKARRAY_BITMAP_SIZE }, () => Array.from({ length: 8 }, () => new BN(0))),
            },
        };
    }
    // transaction
    static async makeCreatePoolInstructionSimple({ makeTxVersion, connection, programId, owner, payer, mint1, mint2, ammConfig, initialPrice, startTime, computeBudgetConfig, lookupTableCache, }) {
        const [mintA, mintB, initPrice] = new BN(mint1.mint.toBuffer()).gt(new BN(mint2.mint.toBuffer()))
            ? [mint2, mint1, new Decimal(1).div(initialPrice)]
            : [mint1, mint2, initialPrice];
        const initialPriceX64 = SqrtPriceMath.priceToSqrtPriceX64(initPrice, mintA.decimals, mintB.decimals);
    ...
    static async makeOpenPositionFromLiquidityInstructionSimple({ makeTxVersion, connection, poolInfo, ownerInfo, amountMaxA, amountMaxB, tickLower, tickUpper, liquidity, associatedOnly = true, checkCreateATAOwner = false, withMetadata = 'create', getEphemeralSigners, computeBudgetConfig, lookupTableCache, }) {
    ...
    static async makeOpenPositionFromBaseInstructionSimple({ connection, poolInfo, ownerInfo, tickLower, tickUpper, base, baseAmount, otherAmountMax, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, withMetadata = 'create', makeTxVersion, lookupTableCache, getEphemeralSigners, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeIncreasePositionFromLiquidityInstructionSimple({ connection, poolInfo, ownerPosition, ownerInfo, amountMaxA, amountMaxB, liquidity, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeIncreasePositionFromBaseInstructionSimple({ connection, poolInfo, ownerPosition, ownerInfo, base, baseAmount, otherAmountMax, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeDecreaseLiquidityInstructionSimple({ connection, poolInfo, ownerPosition, ownerInfo, liquidity, amountMinA, amountMinB, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeSwapBaseInInstructionSimple({ connection, poolInfo, ownerInfo, inputMint, amountIn, amountOutMin, priceLimit, remainingAccounts, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeSwapBaseOutInstructionSimple({ connection, poolInfo, ownerInfo, outputMint, amountOut, amountInMax, priceLimit, remainingAccounts, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeCLosePositionInstructionSimple({ poolInfo, ownerPosition, ownerInfo, makeTxVersion, lookupTableCache, connection, }) {
    ...
    static async makeInitRewardInstructionSimple({ connection, poolInfo, ownerInfo, rewardInfo, chainTime, associatedOnly = true, checkCreateATAOwner = false, makeTxVersion, lookupTableCache, }) {
        ...
        const ownerRewardAccount = await this._selectOrCreateTokenAccount({
            ...
            createInfo: rewardMintUseSOLBalance
                ? {
                    connection,
                    payer: ownerInfo.feePayer,
                    amount: new BN(new Decimal(_baseRewardAmount.toFixed(0)).gte(_baseRewardAmount)
    ...
    static async makeInitRewardsInstructionSimple({ connection, poolInfo, ownerInfo, rewardInfos, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
    ...
    static async makeSetRewardInstructionSimple({ connection, poolInfo, ownerInfo, rewardInfo, chainTime, associatedOnly = true, checkCreateATAOwner = false, makeTxVersion, lookupTableCache, }) {
        logger.assertArgument(rewardInfo.endTime > rewardInfo.openTime, 'reward time error', 'rewardInfo', rewardInfo);
        logger.assertArgument(rewardInfo.openTime > chainTime, 'reward must be paid later', 'rewardInfo', rewardInfo);
        const frontInstructions = [];
        const endInstructions = [];
        ...
        const ownerRewardAccount = await this._selectOrCreateTokenAccount({
            ...
            createInfo: rewardMintUseSOLBalance
                ? {
                    connection,
                    payer: ownerInfo.feePayer,
                    amount: new BN(new Decimal(rewardInfo.perSecond.sub(rewardInfo.endTime - rewardInfo.openTime).toFixed(0)).gte(rewardInfo.perSecond.sub(rewardInfo.endTime - rewardInfo.openTime))
    ...
    static async makeSetRewardsInstructionSimple({ connection, poolInfo, ownerInfo, rewardInfos, chainTime, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeCollectRewardInstructionSimple({ connection, poolInfo, ownerInfo, rewardMint, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeCollectRewardsInstructionSimple({ connection, poolInfo, ownerInfo, rewardMints, associatedOnly = true, checkCreateATAOwner = false, computeBudgetConfig, makeTxVersion, lookupTableCache, }) {
        const frontInstructions = [];
        const endInstructions = [];
        const frontInstructionsType = [];
        const endInstructionsType = [];
    ...
    static async makeHarvestAllRewardInstructionSimple({ connection, fetchPoolInfos, ownerInfo, associatedOnly = true, checkCreateATAOwner = false, makeTxVersion, lookupTableCache, }) {
        ...
        for (const itemInfo of Object.values(fetchPoolInfos)) {
            ...
            const mintAUseSOLBalance = ownerInfo.useSOLBalance && poolInfo.mintA.mint.equals(Token.WSOL.mint);
            const mintBUseSOLBalance = ownerInfo.useSOLBalance && poolInfo.mintB.mint.equals(Token.WSOL.mint);
            const ownerTokenAccountA = ownerMintToAccount[poolInfo.mintA.mint.toString()] ??
                (await this._selectOrCreateTokenAccount({
                    programId: poolInfo.mintA.programId,
                    mint: poolInfo.mintA.mint,
                    tokenAccounts: mintAUseSOLBalance ? [] : ownerInfo.tokenAccounts,
            ...
            const ownerTokenAccountB = ownerMintToAccount[poolInfo.mintB.mint.toString()] ??
                (await this._selectOrCreateTokenAccount({
                    programId: poolInfo.mintB.programId,
                    mint: poolInfo.mintB.mint,
                    tokenAccounts: mintBUseSOLBalance ? [] : ownerInfo.tokenAccounts,
            ...
            for (const itemReward of poolInfo.rewardInfos) {
                const rewardUseSOLBalance = ownerInfo.useSOLBalance && itemReward.tokenMint.equals(Token.WSOL.mint);
                const ownerRewardAccount = ownerMintToAccount[itemReward.tokenMint.toString()] ??
                    (await this._selectOrCreateTokenAccount({
                        programId: itemReward.tokenProgramId,
                        mint: itemReward.tokenMint,
                        tokenAccounts: rewardUseSOLBalance ? [] : ownerInfo.tokenAccounts,
    ...
    // instrument
    static async makeCreatePoolInstructions({ connection, programId, owner, mintA, mintB, ammConfigId, initialPriceX64, startTime, }) {
        ...
        const instructions = [
            ...
            createPoolInstruction(programId, poolId, owner, ammConfigId, observationId.publicKey, mintA.mint, mintAVault, mintA.programId, mintB.mint, mintBVault, mintB.programId, getPdaExBitmapAccount(programId, poolId).publicKey, initialPriceX64, startTime),
        ];
        return {
            address: {
    ...
    static async makeOpenPositionFromLiquidityInstructions({ poolInfo, ownerInfo, tickLower, tickUpper, liquidity, amountMaxA, amountMaxB, withMetadata, getEphemeralSigners, }) {
        const signers = [];
        let nftMintAccount;
        ...
        const ins = openPositionFromLiquidityInstruction(poolInfo.programId, ownerInfo.feePayer, poolInfo.id, ownerInfo.wallet, nftMintAccount, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, poolInfo.mintA.vault, poolInfo.mintB.vault, poolInfo.mintA.mint, poolInfo.mintB.mint, tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.tickSpacing, [
    ...
    static async makeOpenPositionFromBaseInstructions({ poolInfo, ownerInfo, tickLower, tickUpper, base, baseAmount, otherAmountMax, withMetadata, getEphemeralSigners, }) {
        const signers = [];
        let nftMintAccount;
        if (getEphemeralSigners) {
            nftMintAccount = new PublicKey((await getEphemeralSigners(1))[0]);
        ...
        const { publicKey: personalPosition } = getPdaPersonalPositionAddress(poolInfo.programId, nftMintAccount);
        const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(poolInfo.programId, poolInfo.id, tickLower, tickUpper);
        const ins = openPositionFromBaseInstruction(poolInfo.programId, ownerInfo.feePayer, poolInfo.id, ownerInfo.wallet, nftMintAccount, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, poolInfo.mintA.vault, poolInfo.mintB.vault, poolInfo.mintA.mint, poolInfo.mintB.mint, tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, withMetadata, base, baseAmount, otherAmountMax, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.tickSpacing, [
            tickArrayLowerStartIndex,
            tickArrayUpperStartIndex,
        ])
            ? getPdaExBitmapAccount(poolInfo.programId, poolInfo.id).publicKey
    ...
    static makeIncreasePositionFromLiquidityInstructions({ poolInfo, ownerPosition, ownerInfo, liquidity, amountMaxA, amountMaxB, }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    increasePositionFromLiquidityInstruction(poolInfo.programId, ownerInfo.wallet, positionNftAccount, personalPosition, poolInfo.id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, poolInfo.mintA.vault, poolInfo.mintB.vault, poolInfo.mintA.mint, poolInfo.mintB.mint, liquidity, amountMaxA, amountMaxB, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.tickSpacing, [
                        tickArrayLowerStartIndex,
                        tickArrayUpperStartIndex,
                    ])
                        ? getPdaExBitmapAccount(poolInfo.programId, poolInfo.id).publicKey
    ...
    static makeIncreasePositionFromBaseInstructions({ poolInfo, ownerPosition, ownerInfo, base, baseAmount, otherAmountMax, }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    increasePositionFromBaseInstruction(poolInfo.programId, ownerInfo.wallet, positionNftAccount, personalPosition, poolInfo.id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, poolInfo.mintA.vault, poolInfo.mintB.vault, poolInfo.mintA.mint, poolInfo.mintB.mint, base, baseAmount, otherAmountMax, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.tickSpacing, [
                        tickArrayLowerStartIndex,
                        tickArrayUpperStartIndex,
                    ])
                        ? getPdaExBitmapAccount(poolInfo.programId, poolInfo.id).publicKey
    ...
    static makeDecreaseLiquidityInstructions({ poolInfo, ownerPosition, ownerInfo, liquidity, amountMinA, amountMinB, }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    decreaseLiquidityInstruction(poolInfo.programId, ownerInfo.wallet, positionNftAccount, personalPosition, poolInfo.id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, poolInfo.mintA.vault, poolInfo.mintB.vault, poolInfo.mintA.mint, poolInfo.mintB.mint, rewardAccounts, liquidity, amountMinA, amountMinB, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.tickSpacing, [
                        tickArrayLowerStartIndex,
                        tickArrayUpperStartIndex,
                    ])
                        ? getPdaExBitmapAccount(poolInfo.programId, poolInfo.id).publicKey
    ...
    static makeClosePositionInstructions({ poolInfo, ownerInfo, ownerPosition, }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    closePositionInstruction(poolInfo.programId, ownerInfo.wallet, ownerPosition.nftMint, positionNftAccount, personalPosition),
                ],
                signers: [],
    ...
    static makeSwapBaseInInstructions({ poolInfo, ownerInfo, inputMint, amountIn, amountOutMin, sqrtPriceLimitX64, remainingAccounts, }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    swapInstruction(poolInfo.programId, ownerInfo.wallet, poolInfo.id, poolInfo.ammConfig.id, isInputMintA ? ownerInfo.tokenAccountA : ownerInfo.tokenAccountB, isInputMintA ? ownerInfo.tokenAccountB : ownerInfo.tokenAccountA, isInputMintA ? poolInfo.mintA.vault : poolInfo.mintB.vault, isInputMintA ? poolInfo.mintB.vault : poolInfo.mintA.vault, isInputMintA ? poolInfo.mintA.mint : poolInfo.mintB.mint, isInputMintA ? poolInfo.mintB.mint : poolInfo.mintA.mint, remainingAccounts, poolInfo.observationId, amountIn, amountOutMin, sqrtPriceLimitX64, true, getPdaExBitmapAccount(poolInfo.programId, poolInfo.id).publicKey),
                ],
                signers: [],
                lookupTableAddress: [poolInfo.lookupTableAccount].filter((i) => i && !i.equals(PublicKey.default)),
    ...
    static makeInitRewardInstructions({ poolInfo, ownerInfo, rewardInfo, }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    initRewardInstruction(poolInfo.programId, ownerInfo.wallet, poolInfo.id, operationId, poolInfo.ammConfig.id, ownerInfo.tokenAccount, rewardInfo.programId, rewardInfo.mint, poolRewardVault, rewardInfo.openTime, rewardInfo.endTime, rewardInfo.emissionsPerSecondX64),
                ],
                signers: [],
    ...
    static makeSetRewardInstructions({ poolInfo, ownerInfo, rewardInfo, }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    setRewardInstruction(poolInfo.programId, ownerInfo.wallet, poolInfo.id, operationId, poolInfo.ammConfig.id, ownerInfo.tokenAccount, rewardVault, rewardMint, rewardIndex, rewardInfo.openTime, rewardInfo.endTime, rewardInfo.emissionsPerSecondX64),
                ],
                signers: [],
                instructionTypes: [InstructionType.clmmInitReward],
                lookupTableAddress: [poolInfo.lookupTableAccount].filter((i) => i && !i.equals(PublicKey.default)),
    ...
    static makeCollectRewardInstructions({ poolInfo, ownerInfo, rewardMint, }) {
        ...
        return {
            ...
            innerTransaction: {
                instructions: [
                    collectRewardInstruction(poolInfo.programId, ownerInfo.wallet, poolInfo.id, ownerInfo.tokenAccount, rewardVault, rewardMint, rewardIndex),
                ],
                signers: [],
                instructionTypes: [InstructionType.clmmInitReward],
                lookupTableAddress: [poolInfo.lookupTableAccount].filter((i) => i && !i.equals(PublicKey.default)),
    ...
    static getLiquidityAmountOutFromAmountIn({ poolInfo, inputA, tickLower, tickUpper, amount, slippage, add, token2022Infos, epochInfo, amountHasFee, }) {
        ...
        const _amount = new BN(new Decimal(addFeeAmount.amount.sub(addFeeAmount.fee ?? ZERO).toString()).mul(coefficient).toFixed(0));
        let liquidity;
    ...
    static getLiquidityFromAmounts({ poolInfo, tickLower, tickUpper, amountA, amountB, slippage, add, token2022Infos, epochInfo, amountHasFee, }) {
        ...
        const [amountFeeA, amountFeeB] = [
            getTransferAmountFee(_amountA, token2022Infos[poolInfo.mintA.mint.toString()]?.feeConfig, epochInfo, !amountHasFee),
            getTransferAmountFee(_amountB, token2022Infos[poolInfo.mintB.mint.toString()]?.feeConfig, epochInfo, !amountHasFee),
        ];
    ...
    static getAmountsFromLiquidity({ poolInfo, tickLower, tickUpper, liquidity, slippage, add, token2022Infos, epochInfo, amountAddFee, }) {
        ...
        const [amountA, amountB] = [
            getTransferAmountFee(amounts.amountA, token2022Infos[poolInfo.mintA.mint.toString()]?.feeConfig, epochInfo, amountAddFee),
            getTransferAmountFee(amounts.amountB, token2022Infos[poolInfo.mintB.mint.toString()]?.feeConfig, epochInfo, amountAddFee),
        ];
        const [amountSlippageA, amountSlippageB] = [
            getTransferAmountFee(new BN(new Decimal(amounts.amountA.toString()).mul(coefficientRe).toFixed(0)), token2022Infos[poolInfo.mintA.mint.toString()]?.feeConfig, epochInfo, amountAddFee),
            getTransferAmountFee(new BN(new Decimal(amounts.amountB.toString()).mul(coefficientRe).toFixed(0)), token2022Infos[poolInfo.mintB.mint.toString()]?.feeConfig, epochInfo, amountAddFee),
        ];
        return {
            liquidity,
            amountA,
    ...
    static computeAmountOutFormat({ poolInfo, tickArrayCache, token2022Infos, epochInfo, amountIn, currencyOut, slippage, catchLiquidityInsufficient = false, }) {
        ...
        const realAmountIn = {
            ...
            fee: _realAmountIn.fee === undefined
                ? undefined
        ...
        const minAmountOut = {
            ...
            fee: _minAmountOut.fee === undefined
                ? undefined
                : currencyOut instanceof Token
                    ? new TokenAmount(currencyOut, _minAmountOut.fee)
                    : new CurrencyAmount(currencyOut, _minAmountOut.fee),
        };
        const _currentPrice = new Price(amountInIsTokenAmount ? amountIn.token : amountIn.currency, new BN(10).pow(new BN(20 + (amountInIsTokenAmount ? amountIn.token : amountIn.currency).decimals)), currencyOut instanceof Token ? currencyOut : Token.WSOL, currentPrice
            .mul(new Decimal(10 ** (20 + (currencyOut instanceof Token ? currencyOut : Token.WSOL).decimals)))
            .toFixed(0));
        const _executionPrice = new Price(amountInIsTokenAmount ? amountIn.token : amountIn.currency, new BN(10).pow(new BN(20 + (amountInIsTokenAmount ? amountIn.token : amountIn.currency).decimals)), currencyOut instanceof Token ? currencyOut : Token.WSOL, executionPrice
            .mul(new Decimal(10 ** (20 + (currencyOut instanceof Token ? currencyOut : Token.WSOL).decimals)))
            .toFixed(0));
    ...
    static async computeAmountOutAndCheckToken({ connection, poolInfo, tickArrayCache, baseMint, amountIn, slippage, priceLimit = new Decimal(0), catchLiquidityInsufficient = false, }) {
    ...
    static computeAmountOut({ poolInfo, tickArrayCache, baseMint, token2022Infos, epochInfo, amountIn, slippage, priceLimit = new Decimal(0), catchLiquidityInsufficient = false, }) {
        let sqrtPriceLimitX64;
        ...
        const _inputRealAmountIn = getTransferAmountFee(amountIn, token2022Infos[baseMint.toString()]?.feeConfig, epochInfo, false);
        const { allTrade, realTradeAmountIn, expectedAmountOut: _expectedAmountOut, remainingAccounts, executionPrice: _executionPriceX64, feeAmount, } = PoolUtils.getOutputAmountAndRemainAccounts(poolInfo, tickArrayCache, baseMint, _inputRealAmountIn.amount.sub(_inputRealAmountIn.fee ?? ZERO), sqrtPriceLimitX64, catchLiquidityInsufficient);
        const realAmountIn = getTransferAmountFee(realTradeAmountIn, token2022Infos[baseMint.toString()]?.feeConfig, epochInfo, true);
        ...
        const priceImpact = new Percent(new Decimal(_numerator).mul(10 ** 15).toFixed(0), new Decimal(_denominator).mul(10 ** 15).toFixed(0));
        return {
            allTrade,
            realAmountIn,
            amountOut,
    ...
    static computeAmountIn({ poolInfo, tickArrayCache, baseMint, token2022Infos, epochInfo, amountOut, slippage, priceLimit = new Decimal(0), }) {
        let sqrtPriceLimitX64;
        if (priceLimit.equals(new Decimal(0))) {
        ...
        const { expectedAmountIn: _expectedAmountIn, remainingAccounts, executionPrice: _executionPriceX64, feeAmount, } = PoolUtils.getInputAmountAndRemainAccounts(poolInfo, tickArrayCache, baseMint, realAmountOut.amount.sub(realAmountOut.fee ?? ZERO), sqrtPriceLimitX64);
    ...
    static estimateAprsForPriceRangeDelta({ poolInfo, aprType, mintPrice, rewardMintDecimals, liquidity, positionTickLowerIndex, positionTickUpperIndex, chainTime, }) {
        ...
        const poolTvl = new Decimal(poolLiquidityA.toString())
            ...
            .add(new Decimal(poolLiquidityB.toString())
        ...
        const userTvl = new Decimal(userLiquidityA.toString())
            .div(new Decimal(10).pow(mintDecimalsA))
            .mul(mintPriceA.toFixed(mintDecimalsA))
            .add(new Decimal(userLiquidityB.toString())
            .div(new Decimal(10).pow(mintDecimalsB))
            .mul(mintPriceB.toFixed(mintDecimalsB)));
    ...
    // fetch data
    static async fetchMultiplePoolInfos({ connection, poolKeys, ownerInfo, chainTime, batchRequest = false, updateOwnerRewardAndFee = true, }) {
        ...
        if (ownerInfo) {
            ...
            for (const itemAccountInfo of positionAccountInfos) {
                ...
                const tickArrayLowerAddress = TickUtils.getTickArrayAddressByTick(poolsInfo[itemPoolId].state.programId, position.poolId, position.tickLower, poolsInfo[itemPoolId].state.tickSpacing);
                const tickArrayUpperAddress = TickUtils.getTickArrayAddressByTick(poolsInfo[itemPoolId].state.programId, position.poolId, position.tickUpper, poolsInfo[itemPoolId].state.tickSpacing);
    ...
    static async fetchMultiplePoolTickArrays({ connection, poolKeys, batchRequest, }) {
    ...
    static async fetchExBitmaps({ connection, exBitmapAddress, batchRequest, }) {
    ...
    static async getWhiteListMint({ connection, programId }) {
