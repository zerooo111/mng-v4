...
LOCALNET_DIR="${MNG_DIR}/${STACK_DIR_NAME}"
...
FRONTEND_LOCAL_HOST="${FRONTEND_LOCAL_HOST:-127.0.0.1}"
FRONTEND_PORT="${FRONTEND_PORT:-8070}"
...
if [[ "${HARNESS_BACKEND}" == "rust-backend" ]]; then
  DEFAULT_HARNESS_SANITY_INTERVAL_MS=0
  DEFAULT_HARNESS_MARKET_STATS_INTERVAL_MS=0
  DEFAULT_HARNESS_RECONCILE_INTERVAL_MS=30000
else
  DEFAULT_HARNESS_SANITY_INTERVAL_MS=5000
  DEFAULT_HARNESS_MARKET_STATS_INTERVAL_MS=120000
  DEFAULT_HARNESS_RECONCILE_INTERVAL_MS=10000
fi
CONTINUUM_HARNESS_SANITY_INTERVAL_MS="${CONTINUUM_HARNESS_SANITY_INTERVAL_MS:-${DEFAULT_HARNESS_SANITY_INTERVAL_MS}}"
CONTINUUM_HARNESS_MARKET_STATS_INTERVAL_MS="${CONTINUUM_HARNESS_MARKET_STATS_INTERVAL_MS:-${DEFAULT_HARNESS_MARKET_STATS_INTERVAL_MS}}"
CONTINUUM_HARNESS_RECONCILE_INTERVAL_MS="${CONTINUUM_HARNESS_RECONCILE_INTERVAL_MS:-${DEFAULT_HARNESS_RECONCILE_INTERVAL_MS}}"
CONTINUUM_HARNESS_ONCHAIN_CACHE_TTL_MS="${CONTINUUM_HARNESS_ONCHAIN_CACHE_TTL_MS:-5000}"
CONTINUUM_HARNESS_FRONTEND_REFRESH_INTERVAL_MS="${CONTINUUM_HARNESS_FRONTEND_REFRESH_INTERVAL_MS:-10000}"
CONTINUUM_HARNESS_READ_FAILURE_WINDOW_MS="${CONTINUUM_HARNESS_READ_FAILURE_WINDOW_MS:-30000}"
CONTINUUM_HARNESS_READ_FAILURE_MIN_SAMPLES="${CONTINUUM_HARNESS_READ_FAILURE_MIN_SAMPLES:-2}"
CONTINUUM_HARNESS_READ_FAILURE_RATE_THRESHOLD="${CONTINUUM_HARNESS_READ_FAILURE_RATE_THRESHOLD:-0.15}"
CONTINUUM_HARNESS_READ_SLOW_THRESHOLD_MS="${CONTINUUM_HARNESS_READ_SLOW_THRESHOLD_MS:-200}"
CONTINUUM_HARNESS_READ_SLOW_MIN_SAMPLES="${CONTINUUM_HARNESS_READ_SLOW_MIN_SAMPLES:-3}"
CONTINUUM_HARNESS_READ_SLOW_RATE_THRESHOLD="${CONTINUUM_HARNESS_READ_SLOW_RATE_THRESHOLD:-0.20}"
CONTINUUM_HARNESS_READ_IMMEDIATE_FAILOVER_ON_ERROR="${CONTINUUM_HARNESS_READ_IMMEDIATE_FAILOVER_ON_ERROR:-true}"
CONTINUUM_HARNESS_READ_IMMEDIATE_SLOW_THRESHOLD_MS="${CONTINUUM_HARNESS_READ_IMMEDIATE_SLOW_THRESHOLD_MS:-750}"
CTM_RELAYER_LOCAL_STATE="${CTM_RELAYER_LOCAL_STATE:-true}"
...
SUPERVISOR_HEALTH_INTERVAL_SECS="${SUPERVISOR_HEALTH_INTERVAL_SECS:-5}"
SUPERVISOR_HEALTH_FAIL_THRESHOLD="${SUPERVISOR_HEALTH_FAIL_THRESHOLD:-3}"
...
screen_session_name() {
  local service="$1"
  echo "stagin4-${STACK_CLUSTER}-${service}"
}
...
screen_session_is_running() {
  local session="$1"
  screen_cmd -S "${session}" -Q select . >/dev/null 2>&1
}
...
start_detached_service() {
  ...
  if [[ "${USE_SCREEN}" == "true" ]]; then
    local session_name quoted_workdir quoted_logfile quoted_health_url quoted_cmd
    session_name="$(screen_session_name "${session}")"
    quoted_workdir="$(printf '%q' "${workdir}")"
    quoted_logfile="$(printf '%q' "${log_file}")"
    quoted_health_url="$(printf '%q' "${health_url}")"
    quoted_cmd="$(printf '%q ' "$@")"
    screen_cmd -S "${session_name}" -X quit >/dev/null 2>&1 || true
    screen_cmd -DmS "${session_name}" \
      bash -lc "cd ${quoted_workdir} && exec bash $(printf '%q' "${SUPERVISOR_SCRIPT}") --name $(printf '%q' "${session}") --workdir ${quoted_workdir} --log-file ${quoted_logfile} --health-start-secs $(printf '%q' "${SUPERVISOR_HEALTH_START_SECS}") --health-interval-secs $(printf '%q' "${SUPERVISOR_HEALTH_INTERVAL_SECS}") --health-fail-threshold $(printf '%q' "${SUPERVISOR_HEALTH_FAIL_THRESHOLD}") --restart-delay-secs $(printf '%q' "${SUPERVISOR_RESTART_DELAY_SECS}") ${health_url:+--health-url ${quoted_health_url}} -- ${quoted_cmd}" \
      >/dev/null 2>&1 &
    sleep 0.2
...
preflight_binaries() {
  ...
  if [[ "${STACK_CLUSTER}" != "devnet" ]]; then
    ...
    validator_bin="external"
    validator_version="external"
  fi
  ...
  if [[ "${STACK_CLUSTER}" != "devnet" && "${validator_version}" != "${REQUIRED_SOLANA_VERSION}"* ]]; then
...
pid_is_running() {
  ...
  ref="$(cat "${pid_file}")"
  if [[ "${ref}" == screen:* ]]; then
    screen_session_is_running "${ref#screen:}"
    return $?
  fi
...
stop_pid_file() {
  ...
  if pid_is_running "${pid_file}"; then
    ...
    if [[ "${ref}" == screen:* ]]; then
      local session
      session="${ref#screen:}"
      log "Stopping ${name} (screen ${session})"
      screen_cmd -S "${session}" -X quit >/dev/null 2>&1 || true
      for _ in $(seq 1 50); do
        if ! screen_session_is_running "${session}"; then
          break
        fi
        sleep 0.1
      done
...
stop_all() {
  ...
  if [[ "${FRONTEND_HOST}" != "${FRONTEND_LOCAL_HOST}" ]]; then
    pkill -f "vite --host ${FRONTEND_LOCAL_HOST} --port ${FRONTEND_PORT}" 2>/dev/null || true
  fi
}
...
wait_for_rpc() {
  ...
  for _ in $(seq 1 "${max_tries}"); do
    ...
    if curl -s "${SOLANA_URL}" \
      -H 'Content-Type: application/json' \
      -d '{"jsonrpc":"2.0","id":1,"method":"getLatestBlockhash"}' | rg -q '"result"'; then
      return 0
    fi
    sleep 1
  done
...
start_validator() {
  if [[ "${STACK_CLUSTER}" == "devnet" ]]; then
    log "Using external devnet RPC: ${SOLANA_URL}"
    wait_for_rpc
    return 0
  fi
...
deploy_or_preload_program() {
  if [[ "${STACK_CLUSTER}" == "devnet" ]]; then
    program_is_deployed || {
      echo "Program not found on ${STACK_CLUSTER}: ${PROGRAM_ID_EFFECTIVE}" >&2
      return 1
    }
    return 0
  fi
...
bootstrap_local_state() {
  ...
  if [[ "${SKIP_BOOTSTRAP}" == "1" ]]; then
    if [[ ! -f "${E2E_CONFIG_PATH}" ]]; then
      echo "SKIP_BOOTSTRAP=1 but missing config: ${E2E_CONFIG_PATH}" >&2
      return 1
    fi
    log "Skipping bootstrap; using existing config ${E2E_CONFIG_PATH}"
    return 0
  fi
  env \
...
start_harness_and_relayer() {
  ...
  start_detached_service \
    ...
    "http://${HARNESS_BIND_ADDR}/healthz" \
    env \
    STACK_CLUSTER="${STACK_CLUSTER}" \
    TS_NODE_TRANSPILE_ONLY="${TS_NODE_TRANSPILE_ONLY}" \
    CLUSTER_OVERRIDE=devnet \
    ...
    CONTINUUM_HARNESS_SANITY_INTERVAL_MS="${CONTINUUM_HARNESS_SANITY_INTERVAL_MS}" \
    CONTINUUM_HARNESS_MARKET_STATS_INTERVAL_MS="${CONTINUUM_HARNESS_MARKET_STATS_INTERVAL_MS}" \
    CONTINUUM_HARNESS_RECONCILE_INTERVAL_MS="${CONTINUUM_HARNESS_RECONCILE_INTERVAL_MS}" \
    CONTINUUM_HARNESS_ONCHAIN_CACHE_TTL_MS="${CONTINUUM_HARNESS_ONCHAIN_CACHE_TTL_MS}" \
    CONTINUUM_HARNESS_FRONTEND_REFRESH_INTERVAL_MS="${CONTINUUM_HARNESS_FRONTEND_REFRESH_INTERVAL_MS}" \
    CONTINUUM_HARNESS_READ_FAILURE_WINDOW_MS="${CONTINUUM_HARNESS_READ_FAILURE_WINDOW_MS}" \
    CONTINUUM_HARNESS_READ_FAILURE_MIN_SAMPLES="${CONTINUUM_HARNESS_READ_FAILURE_MIN_SAMPLES}" \
    CONTINUUM_HARNESS_READ_FAILURE_RATE_THRESHOLD="${CONTINUUM_HARNESS_READ_FAILURE_RATE_THRESHOLD}" \
    CONTINUUM_HARNESS_READ_SLOW_THRESHOLD_MS="${CONTINUUM_HARNESS_READ_SLOW_THRESHOLD_MS}" \
    CONTINUUM_HARNESS_READ_SLOW_MIN_SAMPLES="${CONTINUUM_HARNESS_READ_SLOW_MIN_SAMPLES}" \
    CONTINUUM_HARNESS_READ_SLOW_RATE_THRESHOLD="${CONTINUUM_HARNESS_READ_SLOW_RATE_THRESHOLD}" \
    CONTINUUM_HARNESS_READ_IMMEDIATE_FAILOVER_ON_ERROR="${CONTINUUM_HARNESS_READ_IMMEDIATE_FAILOVER_ON_ERROR}" \
    CONTINUUM_HARNESS_READ_IMMEDIATE_SLOW_THRESHOLD_MS="${CONTINUUM_HARNESS_READ_IMMEDIATE_SLOW_THRESHOLD_MS}" \
  ...
  if [[ "${CTM_RELAYER_IMPL}" == "rust" ]]; then
    if [[ -x "${EXECUTION_ENGINE_BIN}" ]]; then
      start_detached_service \
        ...
        EXECUTION_QUEUE_CRANK_MAX_ITEMS=1 \
        EXECUTION_QUEUE_CRANK_INTERVAL_MS=25 \
        EXECUTION_QUEUE_CRANK_BUSY_INTERVAL_MS=25 \
        EXECUTION_QUEUE_CRANK_PENDING_TIMEOUT_MS=1000 \
        EXECUTION_QUEUE_CRANK_STATUS_POLL_MS=10 \
        EXECUTION_QUEUE_CRANK_MAX_PENDING_TXS=12 \
        EXECUTION_QUEUE_CRANK_HEAD_LOCK_MS=5 \
        EXECUTION_QUEUE_CRANK_MATCH_HEAD_ONLY=true \
        EXECUTION_QUEUE_CRANK_SKIP_PREFLIGHT=true \
        CTM_RELAYER_QUEUE_SOFT_LIMIT=1024 \
        CTM_RELAYER_QUEUE_GAP_SOFT_LIMIT=1024 \
        CTM_RELAYER_HARNESS_REJECT_MARKET_DRIFT="${CTM_RELAYER_HARNESS_REJECT_MARKET_DRIFT}" \
        CTM_RELAYER_LOCAL_STATE="${CTM_RELAYER_LOCAL_STATE}" \
        CTM_RELAYER_LOCAL_STATE_BOOTSTRAP_URL="${CTM_RELAYER_LOCAL_STATE_BOOTSTRAP_URL}" \
        CTM_RELAYER_LOCAL_STATE_BOOTSTRAP_TIMEOUT_MS="${CTM_RELAYER_LOCAL_STATE_BOOTSTRAP_TIMEOUT_MS}" \
...
start_frontend() {
  cd "${FRONTEND_DIR}"
  local ws_url
  ws_url="$(derive_ws_url "${SOLANA_URL}")"
  start_detached_service \
    "frontend" \
    "${FRONTEND_PID_FILE}" \
    "${FRONTEND_DIR}" \
    "${FRONTEND_RUN_DIR}/frontend.log" \
    "http://${FRONTEND_LOCAL_HOST}:${FRONTEND_PORT}/" \
    env \
    VITE_API_BASE_URL=/harness \
    VITE_RELAY_BRIDGE_URL=/bridge \
    VITE_TIMESCALE_API_URL=/bridge \
    VITE_RPC_URL="${SOLANA_URL}" \
    VITE_WS_URL="${ws_url}" \
    VITE_COMMITMENT=confirmed \
    VITE_MANGO_PROGRAM_ID="${PROGRAM_ID_EFFECTIVE}" \
    VITE_HARNESS_MARKET_ID="${PERP_MARKET_INDEX}" \
    VITE_MANGO_GROUP_PK="$(read_cfg_field group)" \
    VITE_MANGO_EXECUTION_QUEUE_PK="$(read_cfg_field executionQueue)" \
    VITE_MANGO_ACCOUNT_PK="$(read_cfg_field maker.mangoAccount)" \
    VITE_MANGO_DEPOSIT_UI_AMOUNT="${HARNESS_AIRDROP_DEPOSIT_UI_AMOUNT}" \
    VITE_QUOTE_MINT="$(read_cfg_field usdcMint)" \
...
smoke_checks() {
  ...
  curl -fsS "http://${FRONTEND_LOCAL_HOST}:${FRONTEND_PORT}/harness/healthz" >/dev/null
  log "Smoke: relay config via bridge proxy"
  curl -fsS \
    "http://${FRONTEND_LOCAL_HOST}:${FRONTEND_PORT}/bridge/relay/config?owner=${owner}" >/dev/null
  if [[ "${HARNESS_ENABLE_AIRDROP}" == "true" ]]; then
    log "Smoke: harness reports airdrop enabled"
    curl -fsS "http://${FRONTEND_LOCAL_HOST}:${FRONTEND_PORT}/harness/healthz" | rg -q '"airdrop_enabled":true'
  fi
...
status_all() {
  ...
  if curl -fsS "http://${FRONTEND_LOCAL_HOST}:${FRONTEND_PORT}/" >/dev/null 2>&1; then
    echo "  frontend: ok"
  else
    echo "  frontend: fail"
  fi
  if [[ "${CTM_RELAYER_IMPL}" == "rust" ]]; then
    if curl -fsS "http://${CTM_EXECUTION_ENGINE_HTTP_BIND_ADDR}/healthz" >/dev/null 2>&1; then
      echo "  relayer_http: ok"
    else
      echo "  relayer_http: fail"
    fi
  fi
